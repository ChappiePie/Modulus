...
public class AbilityLayerRenderer<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T, M> {
    ...
    public void render(PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, T livingEntity, float limbSwing, float limbSwingAmount, float partialTick, float ageInTicks, float netHeadYaw, float headPitch) {
        CommonUtil.getAbilities(livingEntity).forEach(ability -> ability.clientProperties(c -> c.render(this.renderer, poseStack, bufferSource, packedLight, ability.entity, ((IHasModelProperties) this.getParentModel()).modulus$modelProperties())));
    }
}